...
- 空白分支只保留持久化组织关联，不继承历史、摘录或备注；创建流程支持可选会话名称。
- 会话树统一显示完整分叉、仅携带枝签和空白分支，提供从上向下的全景树、窄面板层级和直接打开会话的节点。
- 枝签排序支持键盘操作、平滑让位和拖动预览；删除与排序提供即时撤销。

### Changed

- 卡片按内容自然排布，保留紧凑的创建新会话、引用和 Side Chat 操作；编辑、置顶和删除收进菜单，批量工具栏只在至少两枚选中时显示。
- 回收站作为搜索框右侧的独立按钮，显示非零数量、恢复入口和对应空状态。
