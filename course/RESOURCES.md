...
- 当前官方教程没有把 Host Remote、独立 Client Module、普通 Session 分叉和直接 LLM 调用串成一个完整的树外插件实例；本课程用当前插件源码补齐这条端到端路径。
