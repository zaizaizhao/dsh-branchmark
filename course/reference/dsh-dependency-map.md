...
| `@deepseek-ai/dsh-client-ui-conversation/client` | Composer/Header/Chat slots、Session input 与 Chat node 类型 | 输入区入口、原生引用插入、lineage badge、fork divider、选区 anchor |
| `@deepseek-ai/dsh-client-ui-input-trigger/client` | `ReferenceInsert`、source registry 与提交 codec | 短引用 Chip、提交时 Clip 上下文序列化 |
| `@deepseek-ai/dsh-client-ui-primitives` | 图标与 `MarkdownText` | 主题一致的图标、Markdown 答案与 reasoning 渲染 |
...
Slot 的声明、owner props 与 inject face 以 DSH 源码为准：[`ui-layout` shell contract](../../../packages/client/ui-layout/src/client/index.ts)、[`ui-sidebar` contract](../../../packages/client/ui-sidebar/src/client/contract/slots.ts) 和 [`ui-conversation` contract](../../../packages/client/ui-conversation/src/client/contract/slots.ts)。
...
标准 Web profile 已装载绝大多数依赖，具体行见 DSH [`dsh-base` patch](../../../packages/bundle/base/cordis.patch.yml) 与 [`dsh-web-app` patch](../../../packages/bundle/web-app/cordis.patch.yml)。如果用户移除任一 required Service，Cordis 会让 BranchMark 保持 pending，而不是在缺少能力时以降级逻辑运行。
