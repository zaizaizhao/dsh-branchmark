...
DSH 官方 [`architecture.md`](../../../docs/architecture.md) 说明：模型适配器、工具、Session log、agent loop 和 UI 都由 Cordis 插件组合，没有必须修改的特权核心。扩展的正确动作是把插件挂到现有 Service 和 Slot 旁边。
...
原始说明见 [`Cordis Primer`](../../../docs/cordis-primer.md)。
...
1. 在 [`dsh-web-app/cordis.patch.yml`](../../../packages/bundle/web-app/cordis.patch.yml) 找出 storage、workspace、client-runtime、ui-layout 和 ui-conversation 行。
2. 在 [`dsh-base/cordis.patch.yml`](../../../packages/bundle/base/cordis.patch.yml) 找出 llm、session、persistence、fs 与 web 行。
