...
分开 face 的直接原因是 Host 和 Client 都会 declaration-merge Cordis `Context`，但同名 key 可能对应不同运行类型。把两者压进一个 TypeScript program 会制造 merge collision；DSH 自身也使用独立 Host/Client aggregate，见官方 [`development.md`](../../../docs/development.md#typescript-project-layout)。
...
打开 [`packages/host/lib/typert.remote-client.d.ts`](../../packages/host/lib/typert.remote-client.d.ts)，确认 `TypertRemoteNamespaceMap` 有 `branchmark` key。生成文件缺失时先修构建，不能在 Client 手写 `ctx.remote` cast 继续开发。
