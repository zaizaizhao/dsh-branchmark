...
`Clip` 的 `source` 与 `excerpt` 没有更新 API，`UpdateClipRequest` 只允许 note、tags、scope 和 pinned。`sortIndex` 不能由单条 update 任意指定，只能由 Host 校验过的完整集合重排产生。`ClipSource` 的两个分支把能力写进类型：
...
`pinnedAt` 与 `sortIndex` 是两个独立的可选字段：前者记录用户是否把 Clip 放入置顶组，后者记录它在当前 active 会话集合或项目集合中的位置。不要从一个数值区间推断置顶，也不要把 `sortIndex` 当作跨 scope 的全局名次；Clip 改变 scope 或置顶状态时会进入另一集合，旧索引必须清除。
...
- tags 不得重复。
- `pinnedAt` 缺失表示未置顶；`sortIndex` 若存在，必须是非负安全整数。
...
重点观察：合法 Clip 的 tags 被规范化；伪造 excerpt 不写入；session/project visibility 隔离；置顶和完整集合顺序可持久化；局部或跨组排序被拒绝；删除 Clip 后 relation/usage 仍存在。
...
5. 为什么增加两个排序字段后仍可保持 domain version 1？

前三题分别对应保存目标、原子快照、来源身份。第四题应选择插件自有索引/schema 演进，不能绕过 storage abstraction。第五题的前提是旧记录缺少字段时仍有明确且安全的解释；如果新布局让旧值产生歧义，就必须升级 schema 并设计迁移。
