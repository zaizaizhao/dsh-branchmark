...
DSH [Web 子系统文档](../../../docs/subsystems/web.md)还说明本地 HTTP fetch backend 默认不阻断 private-network target。若部署需要 fetch，管理员必须选配满足自身 SSRF/network policy 的 provider；插件不能把一个通用 `ctx.web.fetch` 调用宣传为自动隔离内网。
