...
React 与 DSH Browser packages 保持 external，由 `window.__ModuleLoader__` 同步 `require`；Markdown projection 等插件专用依赖则打入 `client.js`。`dsh.client.inject` 描述代码到达顺序，Client Modules 扫描 package、计算 rev，并通过 `/plugins/<id>/client.js?rev=...` 提供文件。官方机制见 [Client Modules 文档](../../../docs/subsystems/client-modules.md)。
...
```sh
tar -tf /absolute/path/to/dsh-branchmark-0.2.0.tgz
```
...
dsh plugin --profile clip-course add /absolute/path/to/dsh-branchmark-0.2.0.tgz
