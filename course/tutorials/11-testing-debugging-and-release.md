...
- 多 Clip 按选择顺序头部插入，draft mirror token 在不改变周围文字时恢复，单枚移除不破坏相邻 occurrence。
- 六项批量命令胶囊、跨置顶组拖拽拒绝，以及固定卡片、专注阅读、置顶与拖拽控件的静态意图。
...
| 备注/标签/批量/回收站 | 命令胶囊只展开所选动作，mutation 后查询一致，永久删除保留 child usage |
| 置顶、排序与阅读 | 置顶组在前；同组拖拽跨重启保留；筛选/回收站/跨组不重排；折叠、展开与 Modal 均不改变原文 |
| Composer | 多枚原生引用保持选择顺序且绝不自动发送；重新绑定后 token 恢复为 Chip；缺失 token 留在 draft；逐条移除不破坏问题或相邻 Chip |
...
- Client 纯测试不能证明真实 Composer draft mirror 的刷新时序、指针捕获拖拽和 Modal focus trap；这些仍需 Browser smoke。
