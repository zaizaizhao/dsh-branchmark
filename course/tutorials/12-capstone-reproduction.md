...
- 原文与来源不可编辑，note/tags/scope 可变；支持回收站与永久删除。
- 选区能处理渲染 Markdown，同消息连续选择一条、跨消息拆分。
- 显式 Clip 可放入 Composer，但不自动发送。
...
加入 session/project visibility、note、tags、scope、search、trash、permanent delete 与 batch prevalidation。保持 source/excerpt 没有 update path；把 durable record frozen，用 table `update` 替换。
...
不要一开始写完整视觉样式。先用 Client pure tests证明 candidate，Host 再次证明 source；最后增加主题、grid/list、search/tags、resize、rail/hidden 和 accessibility。

检查点：普通、重复文本、链接/强调、跨消息、未完成 assistant 各有明确结果；真实 UI 不挡住宿主其他 overlay；localStorage 中没有 Clip 正文。

## 7. 接入 Composer

生成用户可见 quote prefix，调用 `setDraft` 而非 prompt。Controller 保存 items 与生成 prefix，只在当前 draft 仍匹配 prefix 时允许 chip remove。

检查点：加入 Clip 后没有网络/模型提交；用户追加问题后删除一个 chip，问题保留；手改 prefix 后插件拒绝自动替换。
...
2. 提升另一条为 project Clip，在新 Session 中证明可见。
3. 选 primary + 第二条 Clip，full-fork 到第 2 轮，创建但不发送；展示 parent tree、divider、recall 和空 Composer。
4. 用相同 Clip clips-only 并创建并发送；展示它没有 parent。
5. 启动两个 Side Chat，一个换模型、一个调用 project search；最小化后恢复。
6. 停止一个回答，关闭另一个，保存第一条临时回答为 Clip。
7. 删除原 Clip，展示两个普通 child 不受影响且 relation usage 仍可读。
8. 重启 DSH，展示 Clip/Session 仍在，Side Chat 已消失。
