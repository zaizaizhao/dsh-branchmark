...
export function BranchMarkSidebarButton({
  wide,
  useSessions,
  useWorkspaces,
  controller,
  ...
  useEffect(() => {
    ...
    void client.list({ workspaceId, visibility: 'project-library' }).then(
      (value) => {
        if (active) setCount(value.clips.filter((clip) => clip.scope === 'project').length)
      },
      () => {
  ...
  return (
    <button
      ...
      onClick={() => {
        if (workspaceId !== undefined) controller.openDock('project')
      }}
    >
      <span className="dbm-sidebar-nav-icon">
        <BranchMarkLogo compact size={wide ? 15 : 16} />
      </span>
      {wide && <span className="dbm-sidebar-nav-label">枝签</span>}
      {count !== undefined && count > 0 && (
        <span className="dbm-sidebar-nav-count">{count > 99 ? '99+' : String(count)}</span>
      )}
    </button>
  )
}
...
export function BranchMarkDrawerButton({
  sessionId,
  useInput,
  inputActions,
  controller,
  ...
  return (
    <div className="dbm-composer-reference" ref={root}>
      <button
        ...
        data-active={open}
        title={
          references.length === 0 ? '引用本会话枝签' : `管理已引用的 ${String(references.length)} 枚枝签`
        }
      ...
      {open && (
        <section className="dbm-reference-popover" aria-label="当前消息引用的枝签">
          <header>
            <strong>已引用 {String(references.length)} 枚枝签</strong>
            <small>发送前才会展开为模型上下文</small>
          </header>
            ...
            {references.map((occurrence) => (
              <div className="dbm-reference-row" key={occurrence.occurrenceId}>
                ...
                <button
                  ...
                  onClick={() => {
                    remove(occurrence.occurrenceId)
                  }}
                >
          ...
          <button
            type="button"
            className="dbm-reference-manage"
            onClick={() => {
              setOpen(false)
              controller.openDock('session')
            }}
          >
            打开本会话枝签
          </button>
