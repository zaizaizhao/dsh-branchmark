...
export const clipTextRangeSchema = z
  .object({
    start: safeInteger,
    end: safeInteger,
  })
...
export const derivedSessionRelationSchema = z
  .object({
    derivedSessionId: sessionIdSchema,
    workspaceId: workspaceIdSchema,
    mode: z.enum(['full-fork', 'clips-only', 'blank']),
    ...
    attachedClipIds: z.array(clipIdSchema),
    createdAt: nonEmptyString,
  })
  .superRefine((relation, ctx) => {
    ...
    if (
      sourceFields.some((value) => value !== undefined) !== expected ||
    ...
    if ((relation.mode === 'blank') !== (relation.attachedClipIds.length === 0)) {
      ctx.addIssue({
        code: 'custom',
        path: ['attachedClipIds'],
        message: 'only blank branches have no attachments',
      })
    }
    if (relation.mode === 'blank' && relation.parentSessionId === undefined) {
      ctx.addIssue({
        code: 'custom',
        path: ['parentSessionId'],
        message: 'blank branches require an organizational parent',
      })
    }
    if (relation.parentSessionId === relation.derivedSessionId) {
      ctx.addIssue({
        code: 'custom',
        path: ['parentSessionId'],
    ...
    if (
      ...
      relation.parentSessionId !== relation.sourceSessionId
    ) {
      ctx.addIssue({
        code: 'custom',
        path: ['parentSessionId'],
        message: 'full-fork parent must match its source',
      })
    }
...
export const derivedSessionRecordSchema = z
  .object({
    relation: derivedSessionRelationSchema,
    usages: z.array(clipUsageSchema),
  })
  .superRefine((record, ctx) => {
    if (record.usages.some((usage) => usage.derivedSessionId !== record.relation.derivedSessionId)) {
      ctx.addIssue({
        code: 'custom',
        path: ['usages'],
        message: 'usage Session ids must match their relation',
      })
    }
    if (
      record.usages.map((usage) => usage.clipId).join('\0') !== record.relation.attachedClipIds.join('\0')
    ) {
      ctx.addIssue({
        code: 'custom',
        path: ['usages'],
        message: 'usage Clip ids must match relation attachment order',
      })
    }
